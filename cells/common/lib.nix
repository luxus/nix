...
let
  ...
  defaultSources =
    if l.pathExists defaultSourcesPath then nixpkgs.callPackage defaultSourcesPath { } else null;
in
{
  ...
  importPackages =
    ...
    let
      ...
      pkgs = l.mapAttrs (n: v: nixpkgs.callPackage v (args // { inherit sources; })) paths;
    in
    pkgs;
}

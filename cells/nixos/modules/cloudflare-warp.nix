...
with builtins // lib;
{
  options = {
    services.cloudflare-warp = {
      ...
      package = mkOption {
        type = types.package;
        default = cell.packages.cloudflare-warp;
  ...
  config = mkIf cfg.enable {
    ...
    security.pki = mkIf (cfg.certificate != null) { certificateFiles = [ cfg.certificate ]; };

    networking.firewall = mkIf cfg.openFirewall { allowedUDPPorts = [ cfg.udpPort ]; };
